...
#        ax.set_ylim([0,max(max(cy), max(ey)) + 50])
    ...
    def generate_round_charts(datas, basedir=''):
        ...
        for g in datas['gamma'].values:
            for b in datas['beta'].values:
                ...
                if g == 1.0:
                    ...
                    sz_with_g1_each.append(filterd['runOnSmartphoneTot'].values[0, -1] / numSmartphoneNode)
        # 3D chart
        fig = plt.figure(figsize=(6,4))
        ax = fig.add_subplot(1, 1, 1, projection='3d')
        ax.set_title("Num of Protelis rounds")
        ax.set_xlabel("$P_{e}$")
        ax.set_ylabel("$P_{l}$")
        ...
        make_double_line_chart(b_axis_with_g1, cz_with_g1, 'OnCloud', colors[2], sz_with_g1_all, 'OnAllThermostats', colors[1], xlabel='$P_{l}$', ylabel='num of round', title='Num of Protelis rounds', filename=filename)
        # 2D by beta each smartphoneplt.tight_layout()
        filename = f'{basedir}{separator}roundsByPl-eachThermostat.pdf'
        make_double_line_chart(b_axis_with_g1, cz_with_g1, 'OnCloud', colors[2], sz_with_g1_each, 'OnEachThermostat', colors[1], xlabel='$P_{l}$', ylabel='num of round', title='Num of Protelis rounds', filename=filename)
        # 2D by gamma
        filename = f'{basedir}{separator}roundsByPe.pdf'
        make_double_line_chart(g_axis_with_b1, cz_with_b1, 'OnCloud', colors[2], ez_with_b1, 'OnEdge', colors[0], xlabel='$P_{e}$', ylabel='num of round', title='Num of Protelis rounds', filename=filename)
